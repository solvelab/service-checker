...
```bash
docker pull ghcr.io/solvelab/service-checker:v2.7.0
```
...
To pin a release, set `SERVICE_MONITOR_IMAGE_TAG` in `.env` to the current release tag (for example, `v2.7.0`):
```bash
SERVICE_MONITOR_IMAGE_TAG=v2.7.0
```
