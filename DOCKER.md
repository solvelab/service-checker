...
```bash
docker pull ghcr.io/solvelab/service-checker:v1.1.1
```
...
To pin a release, set `SERVICE_MONITOR_IMAGE_TAG` in `.env` to the current release tag (for example, `v1.1.1`):
```bash
SERVICE_MONITOR_IMAGE_TAG=v1.1.1
```
