...
class GitHubStatusMonitor:
    ...
    async def check(
        ...
        if self.config is None:
            raise RuntimeError("github monitor not configured")
        ...
        if rule_status == MonitorStatus.ALERT:
            ...
            return MonitorResult(
                ...
                message="github status degraded",
    ...
    async def _fetch_extra(
        ...
        try:
            ...
            logger.debug(
                "github enrichment fetch failed",
...
def get_monitor(slug: str = "github") -> GitHubStatusMonitor:
    return GitHubStatusMonitor(slug=slug)
